    ...
    <div className="min-h-screen relative flex items-center justify-center p-4">
      ...
      <div className="max-w-[440px] w-full p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] glass-panel relative z-10 overflow-hidden animate-in fade-in zoom-in duration-700">
        ...
        <div className="text-center mb-8 md:mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-black text-white tracking-tighter mb-2">Create Account</h1>
          <p className="text-slate-400 font-medium text-sm md:text-base">Start your EduConnect learning experience.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
          <div className="space-y-2">
            <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-1">Full Name</label>
            <div className="relative group">
              ...
              <input
                ...
                required
                className="w-full pl-10 md:pl-12 pr-4 py-3 md:py-3.5 bg-white/5 border border-white/10 rounded-xl md:rounded-2xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 focus:bg-white/10 outline-none transition-all text-white placeholder:text-slate-600 text-sm md:text-base"
          ...
          <div className="space-y-3">
            <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-1">Choose Role</label>
            <div className="grid grid-cols-2 gap-3 md:gap-4">
              <button
                type="button"
                onClick={() => setFormData({ ...formData, role: 'student' })}
                className={`p-3 md:p-4 rounded-xl md:rounded-2xl border transition-all flex flex-col items-center gap-1.5 md:gap-2 ${
                  formData.role === 'student' 
                    ? 'bg-indigo-600/20 border-indigo-500 text-white indigo-glow' 
                    : 'bg-white/5 border-white/10 text-slate-400 hover:bg-white/10'
                }`}
              >
                <GraduationCap className={`w-5 h-5 md:w-6 md:h-6 ${formData.role === 'student' ? 'text-indigo-400' : ''}`} />
                <span className="text-[10px] font-bold uppercase tracking-widest leading-none">Student</span>
              </button>
              <button
                type="button"
              ...
              >
                <Presentation className={`w-5 h-5 md:w-6 md:h-6 ${formData.role === 'teacher' ? 'text-indigo-400' : ''}`} />
                <span className="text-[10px] font-bold uppercase tracking-widest leading-none">Teacher</span>
              </button>
            </div>
          </div>
          ...
          >
            {isLoading ? <Loader2 className="h-5 w-5 md:h-6 md:w-6 animate-spin" /> : 'Register'}
          </button>
        </form>

        <p className="mt-8 md:mt-10 text-center text-xs md:text-sm font-medium text-slate-400">
          Already have an account?{' '}
          <Link to="/login" className="text-indigo-400 hover:text-indigo-300 font-bold underline-offset-4 hover:underline transition-all">
            Sign In
          </Link>
        </p>
      </div>
